...
const PATH = "/conversions/steps-to-km";
const CANONICAL = "/tools/step-distance-calculator";
...
export async function generateMetadata({ params }: LangPageProps): Promise<Metadata> {
  ...
  return buildPageMetadata({
    ...
    meta: withMetaVars(t.stepsToKm.meta, {
      steps: num(10000),
...
export default async function StepsToKmCategoryPage({ params }: LangPageProps) {
  const locale = await getLocale(params);
  const t = await loadConversionHubsMessages(locale);
  const page = t.stepsToKm;
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <p className="text-lg text-neutral-600 dark:text-neutral-400 mb-10 max-w-2xl">
            {rich(page.intro, {
              calculator: (
                <Link
                  href={localizePath(locale, CANONICAL)}
          ...
          <section className="rounded-3xl bg-gradient-to-br from-[#ED772F]/10 to-[#ED772F]/5 border border-[#ED772F]/20 p-6 md:p-8 mb-10">
            ...
            <p className="text-neutral-700 dark:text-neutral-300 mb-2">
              <code className="px-2 py-1 rounded bg-white/60 dark:bg-black/30 font-mono text-sm">
                {interpolate(page.formula, { factor: formatDecimal(0.00076, locale, 5) })}
              </code>
            </p>
            <p className="text-sm text-neutral-600 dark:text-neutral-400">
              {interpolate(page.formulaNote, {
                stride: num(76),
                perKm: num(100000),
                feet: formatDecimal(2.5, locale, 1),
              })}
            </p>
          </section>
                  ...
                  {rows.map((row) => (
                    ...
                    >
                      ...
                      <td className="p-3 text-right">
                        <Link
                          href={localizePath(locale, `/conversions/steps-to-km/${row.steps}`)}
                          className="text-[#ED772F] hover:underline"
                        >
