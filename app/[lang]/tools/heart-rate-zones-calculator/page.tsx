...
import { loadHeartRateZonesCalculatorMessages } from "@/lib/i18n/messages/tool-pages/heart-rate-zones-calculator";
import { buildPageMetadata, getLocale, type LangPageProps } from "@/lib/i18n/page";

const SLUG = "heart-rate-zones-calculator";
...
export async function generateMetadata({ params }: LangPageProps): Promise<Metadata> {
  ...
  const t = await loadHeartRateZonesCalculatorMessages(locale);
  return buildPageMetadata({
    locale,
    path: PATH,
    meta: t.meta,
    ogImage: "/og/heart-rate-zones-calculator.png",
  });
}

export default async function HeartRateZonesCalculatorPage({ params }: LangPageProps) {
  const locale = await getLocale(params);
  const t = await loadHeartRateZonesCalculatorMessages(locale);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-neutral-950">
      ...
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4 max-w-3xl">
          <HeartRateZonesCalculatorClient t={t.calculator} locale={locale} />
        </div>
      </section>
