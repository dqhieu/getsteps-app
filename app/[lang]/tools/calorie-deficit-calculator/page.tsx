...
import { loadCalorieDeficitMessages } from "@/lib/i18n/messages/tool-pages/calorie-deficit-calculator";
...
const RELATED = [
  ...
  { title: "Body Fat Percentage Calculator", href: "/tools/body-fat-calculator" },
...
export async function generateMetadata({ params }: LangPageProps): Promise<Metadata> {
  ...
  const t = await loadCalorieDeficitMessages(locale);
  return buildPageMetadata({
    locale,
    path: PATH,
    meta: t.meta,
    ogImage: "/og/calorie-deficit-calculator.png",
  });
}

export default async function CalorieDeficitCalculatorPage({ params }: LangPageProps) {
  const locale = await getLocale(params);
  const t = await loadCalorieDeficitMessages(locale);
    ...
    <div className="min-h-screen bg-white dark:bg-neutral-950">
      ...
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4 max-w-5xl">
          <CalorieDeficitCalculatorClient t={t.calculator} locale={locale} />
        </div>
      </section>
