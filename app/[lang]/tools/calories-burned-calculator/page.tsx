...
import { loadCaloriesBurnedMessages } from "@/lib/i18n/messages/tool-pages/calories-burned-calculator";
...
export async function generateMetadata({ params }: LangPageProps): Promise<Metadata> {
  ...
  const t = await loadCaloriesBurnedMessages(locale);
...
export default async function CaloriesBurnedCalculatorPage({ params }: LangPageProps) {
  const locale = await getLocale(params);
  const t = await loadCaloriesBurnedMessages(locale);
          ...
          <div className="bg-white dark:bg-neutral-800/50 rounded-2xl p-6 md:p-8 border border-neutral-200 dark:border-neutral-700/50">
            ...
            <div className="space-y-6 text-neutral-600 dark:text-neutral-400">
              ...
              <div className="bg-neutral-50 dark:bg-neutral-700/30 rounded-xl p-4">
                <h3 className="font-medium text-neutral-900 dark:text-white mb-2">{t.info.formulaTitle}</h3>
                <ul className="space-y-2 text-sm">
                  <li>
                    <strong>{t.info.perMinuteLabel}</strong> {t.info.perMinute}
                  </li>
                  <li>
                    <strong>{t.info.netLabel}</strong> {t.info.net}
                  </li>
                  <li>{t.info.oneMet}</li>
                  <li>
              ...
              <p className="text-sm">{t.info.walkingNote}</p>
            </div>
