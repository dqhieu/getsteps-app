...
import { loadPaceToSpeedConverterMessages } from "@/lib/i18n/messages/tool-pages/pace-to-speed-converter";
import { buildPageMetadata, getLocale, type LangPageProps } from "@/lib/i18n/page";

const SLUG = "pace-to-speed-converter";
const PATH = `/tools/${SLUG}`;

const RELATED = [
  { title: "Running Pace Calculator", href: "/tools/running-pace-calculator" },
  { title: "Marathon Race Predictor", href: "/tools/marathon-pace-predictor" },
  { title: "Race Time Predictor", href: "/tools/race-time-predictor" },
];

export async function generateMetadata({ params }: LangPageProps): Promise<Metadata> {
  const locale = await getLocale(params);
  const t = await loadPaceToSpeedConverterMessages(locale);
  return buildPageMetadata({
    locale,
    path: PATH,
    meta: t.meta,
    ogImage: "/og/pace-to-speed-converter.png",
  });
}

export default async function PaceToSpeedConverterPage({ params }: LangPageProps) {
  const locale = await getLocale(params);
  const t = await loadPaceToSpeedConverterMessages(locale);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-neutral-950">
      ...
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4 max-w-3xl">
          <PaceSpeedConverterClient t={t.calculator} locale={locale} />
        </div>
      </section>
