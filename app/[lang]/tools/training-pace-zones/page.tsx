...
import { loadTrainingPaceZonesMessages } from "@/lib/i18n/messages/tool-pages/training-pace-zones";
import { buildPageMetadata, getLocale, type LangPageProps } from "@/lib/i18n/page";

const SLUG = "training-pace-zones";
...
export async function generateMetadata({ params }: LangPageProps): Promise<Metadata> {
  ...
  const t = await loadTrainingPaceZonesMessages(locale);
  return buildPageMetadata({
    locale,
    path: PATH,
    meta: t.meta,
    ogImage: "/og/training-pace-zones.png",
  });
}

export default async function TrainingPaceZonesPage({ params }: LangPageProps) {
  const locale = await getLocale(params);
  const t = await loadTrainingPaceZonesMessages(locale);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-neutral-950">
      ...
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4 max-w-3xl">
          <TrainingPaceZonesCalculatorClient t={t.calculator} locale={locale} />
        </div>
      </section>
