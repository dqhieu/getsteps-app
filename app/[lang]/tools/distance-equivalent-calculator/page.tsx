...
import { loadDistanceEquivalentCalculatorMessages } from "@/lib/i18n/messages/tool-pages/distance-equivalent-calculator";
...
const SLUG = "distance-equivalent-calculator";
...
export async function generateMetadata({ params }: LangPageProps): Promise<Metadata> {
  ...
  const t = await loadDistanceEquivalentCalculatorMessages(locale);
  return buildPageMetadata({
    locale,
    path: PATH,
    meta: t.meta,
    ogImage: "/og/distance-equivalent-calculator.png",
  });
}

export default async function DistanceEquivalentCalculatorPage({ params }: LangPageProps) {
  const locale = await getLocale(params);
  const t = await loadDistanceEquivalentCalculatorMessages(locale);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-neutral-950">
      ...
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4 max-w-3xl">
          <DistanceEquivalentCalculatorClient t={t.calculator} locale={locale} />
        </div>
      </section>
