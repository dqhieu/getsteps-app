...
import { loadMarathonPacePredictorMessages } from "@/lib/i18n/messages/tool-pages/marathon-pace-predictor";
import { buildPageMetadata, getLocale, type LangPageProps } from "@/lib/i18n/page";

const SLUG = "marathon-pace-predictor";
...
export async function generateMetadata({ params }: LangPageProps): Promise<Metadata> {
  ...
  const t = await loadMarathonPacePredictorMessages(locale);
  return buildPageMetadata({
    locale,
    path: PATH,
    meta: t.meta,
    ogImage: "/og/marathon-pace-predictor.png",
  });
}

export default async function MarathonPacePredictorPage({ params }: LangPageProps) {
  const locale = await getLocale(params);
  const t = await loadMarathonPacePredictorMessages(locale);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-neutral-950">
      ...
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4 max-w-3xl">
          <MarathonRacePredictorClient t={t.calculator} locale={locale} />
        </div>
      </section>
          ...
          <div className="bg-white dark:bg-neutral-800/50 rounded-2xl p-6 md:p-8 border border-neutral-200 dark:border-neutral-700/50">
            ...
            <div className="space-y-4 text-neutral-600 dark:text-neutral-400">
              <div className="border-t border-neutral-200 dark:border-neutral-700 pt-6">
                <h3 className="font-medium text-neutral-900 dark:text-white mb-3">{t.info.faqTitle}</h3>
