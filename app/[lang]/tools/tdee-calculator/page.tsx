...
import { loadTdeeCalculatorMessages } from "@/lib/i18n/messages/tool-pages/tdee-calculator";
...
export async function generateMetadata({ params }: LangPageProps): Promise<Metadata> {
  ...
  const t = await loadTdeeCalculatorMessages(locale);
  return buildPageMetadata({
    locale,
    path: PATH,
    meta: t.meta,
    ogImage: "/og/tdee-calculator.png",
  });
}

export default async function TDEECalculatorPage({ params }: LangPageProps) {
  const locale = await getLocale(params);
  const t = await loadTdeeCalculatorMessages(locale);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-neutral-950">
      ...
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4 max-w-3xl">
          <TDEECalculatorClient t={t.calculator} locale={locale} />
        </div>
      </section>
