...
import { loadRaceTimePredictorMessages } from "@/lib/i18n/messages/tool-pages/race-time-predictor";
import { buildPageMetadata, getLocale, type LangPageProps } from "@/lib/i18n/page";

const SLUG = "race-time-predictor";
...
export async function generateMetadata({ params }: LangPageProps): Promise<Metadata> {
  ...
  const t = await loadRaceTimePredictorMessages(locale);
  return buildPageMetadata({
    locale,
    path: PATH,
    meta: t.meta,
    ogImage: "/og/race-time-predictor.png",
  });
}

export default async function RaceTimePredictorPage({ params }: LangPageProps) {
  const locale = await getLocale(params);
  const t = await loadRaceTimePredictorMessages(locale);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-neutral-950">
      ...
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4 max-w-3xl">
          <RaceTimePredictorClient t={t.calculator} locale={locale} />
        </div>
      </section>
