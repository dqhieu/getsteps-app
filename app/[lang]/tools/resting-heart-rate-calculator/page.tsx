...
import { loadRestingHeartRateCalculatorMessages } from "@/lib/i18n/messages/tool-pages/resting-heart-rate-calculator";
import { buildPageMetadata, getLocale, type LangPageProps } from "@/lib/i18n/page";

const SLUG = "resting-heart-rate-calculator";
...
export async function generateMetadata({ params }: LangPageProps): Promise<Metadata> {
  ...
  const t = await loadRestingHeartRateCalculatorMessages(locale);
  return buildPageMetadata({
    locale,
    path: PATH,
    meta: t.meta,
    ogImage: "/og/resting-heart-rate-calculator.png",
  });
}

export default async function RestingHeartRateCalculatorPage({ params }: LangPageProps) {
  const locale = await getLocale(params);
  const t = await loadRestingHeartRateCalculatorMessages(locale);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-neutral-950">
      ...
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4 max-w-3xl">
          <RestingHeartRateCalculatorClient t={t.calculator} locale={locale} />
        </div>
      </section>
