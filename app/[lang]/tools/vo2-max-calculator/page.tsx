...
import { loadVo2MaxCalculatorMessages } from "@/lib/i18n/messages/tool-pages/vo2-max-calculator";
...
export async function generateMetadata({ params }: LangPageProps): Promise<Metadata> {
  ...
  return buildPageMetadata({
    ...
    path: PATH,
    meta: t.meta,
    ogImage: "/og/vo2-max-calculator.png",
  });
}

export default async function Vo2MaxCalculatorPage({ params }: LangPageProps) {
  const locale = await getLocale(params);
  const t = await loadVo2MaxCalculatorMessages(locale);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-neutral-950">
      ...
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4 max-w-3xl">
          <Vo2MaxCalculatorClient t={t.calculator} locale={locale} />
        </div>
      </section>
