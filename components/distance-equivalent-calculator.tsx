...
export function DistanceEquivalentCalculator({
  ...
  const distCard = (label: string, cardValue: string) => (
    <div className="bg-neutral-50 dark:bg-neutral-900/50 rounded-xl p-4 text-center">
      ...
      <p className="text-base font-bold text-neutral-900 dark:text-white font-mono">{cardValue}</p>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white dark:bg-neutral-800/50 rounded-2xl p-6 border border-neutral-200 dark:border-neutral-700/50">
        <h2 className="text-lg font-semibold text-neutral-900 dark:text-white mb-4">{t.enterDistance}</h2>
      ...
      <div className="bg-white dark:bg-neutral-800/50 rounded-2xl p-6 border border-neutral-200 dark:border-neutral-700/50">
        <h3 className="text-base font-semibold text-neutral-900 dark:text-white mb-4">{t.equivalents}</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          ...
          <div className="bg-[#ED772F]/10 dark:bg-[#ED772F]/20 rounded-xl p-4 text-center">
            <p className="text-xs text-[#ED772F] mb-1 font-medium">{t.approxSteps}</p>
      ...
      <div className="bg-white dark:bg-neutral-800/50 rounded-2xl p-6 border border-neutral-200 dark:border-neutral-700/50">
        <h3 className="text-base font-semibold text-neutral-900 dark:text-white mb-4">{t.context}</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-neutral-200 dark:border-neutral-700">
                <th className="text-left py-2 text-neutral-500 dark:text-neutral-400 font-medium">{t.activity}</th>
                <th className="text-center py-2 text-neutral-500 dark:text-neutral-400 font-medium">{t.speed}</th>
                <th className="text-center py-2 text-neutral-500 dark:text-neutral-400 font-medium">{t.time}</th>
                <th className="text-right py-2 text-neutral-500 dark:text-neutral-400 font-medium">{t.calories}</th>
              </tr>
            </thead>
            <tbody>
              {ACTIVITIES.map((id, index) => (
                <tr
                  key={id}
                  className={index < ACTIVITIES.length - 1 ? "border-b border-neutral-100 dark:border-neutral-700/50" : undefined}
                >
                  <td className="py-3 font-medium text-neutral-900 dark:text-white">{t.activities[id]}</td>
                  <td className="py-3 text-center text-neutral-600 dark:text-neutral-400">{t.speeds[id]}</td>
                  <td className="py-3 text-center font-mono text-neutral-900 dark:text-white">{times[id]}</td>
                  <td className="py-3 text-right text-neutral-900 dark:text-white">{calories[id]}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-2">{t.calorieNote}</p>
      </div>
    </div>
  );
}
