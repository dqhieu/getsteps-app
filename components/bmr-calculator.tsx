...
export function BMRCalculator({ resultCta }: { resultCta?: ReactNode } = {}) {
        ...
        <div className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
              Body Fat % <span className="font-normal text-neutral-500 dark:text-neutral-400">(optional: unlocks Katch-McArdle)</span>
            </label>
            <div className="relative max-w-xs">
              <input type="number" value={bodyFat} placeholder="e.g. 20"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {ACTIVITY_LEVELS.map((level) => (
                <button key={level} onClick={() => setActivity(level)}
                  title={ACTIVITY_DESCRIPTIONS[level]}
          ...
          <div className="bg-white dark:bg-neutral-800/50 rounded-2xl p-6 border border-neutral-200 dark:border-neutral-700/50">
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              <div className="rounded-xl p-4 bg-[#ED772F]/10 dark:bg-[#ED772F]/20 border border-[#ED772F]/30 text-center">
                <p className="text-xs text-[#ED772F] font-medium mb-1">BMR</p>
                <p className="text-3xl font-bold text-neutral-900 dark:text-white">{result.bmr.toLocaleString()}</p>
                <p className="text-xs text-neutral-500 dark:text-neutral-400">calories/day at rest</p>
              </div>
              <div className="rounded-xl p-4 bg-neutral-50 dark:bg-neutral-700/30 text-center">
                <p className="text-xs text-neutral-500 dark:text-neutral-400 mb-1">Maintenance</p>
                <p className="text-3xl font-bold text-neutral-900 dark:text-white">{selectedMaintenance.toLocaleString()}</p>
                <p className="text-xs text-neutral-500 dark:text-neutral-400">at {ACTIVITY_LABELS[activity].toLowerCase()}</p>
              </div>
            </div>

            {result.leanBodyMassKg !== null && (
              <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-6">
                Lean body mass: <strong className="text-neutral-900 dark:text-white">{result.leanBodyMassKg} kg</strong>.
                Because you supplied a body fat percentage, the headline number uses Katch-McArdle.
              </p>
            )}

            <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-6">
              Even sitting still all day, BMR accounts for about {result.bmrShareOfSedentary}% of the calories you burn.
            </p>

            <div className="mb-6">
              <p className="text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-3">All Three Formulas</p>
                  ...
                  <div key={f.key}
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        <span className="text-sm font-medium text-neutral-900 dark:text-white">{f.name}</span>
                        <span className="ml-2 text-xs text-neutral-500 dark:text-neutral-400">{f.year}</span>
                        {f.key === result.primaryFormula && (
                          <span className="ml-2 text-xs text-[#ED772F] font-semibold">Used</span>
                        )}
                      </div>
                      <span className="text-sm font-bold text-neutral-900 dark:text-white whitespace-nowrap">
                        {f.available ? `${f.bmr.toLocaleString()} cal` : "—"}
                      </span>
                    </div>
                    <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{f.note}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              <p className="text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-3">Daily Calories by Activity Level</p>
              <div className="space-y-2">
                {result.maintenance.map((m) => (
                  <div key={m.level}
                    ...
                    <div>
                      <span className="text-sm font-medium text-neutral-900 dark:text-white">{ACTIVITY_LABELS[m.level]}</span>
                      <p className="text-xs text-neutral-500 dark:text-neutral-400">{ACTIVITY_DESCRIPTIONS[m.level]}</p>
                    </div>
                    <span className="text-sm font-bold text-neutral-900 dark:text-white whitespace-nowrap">{m.calories.toLocaleString()} cal</span>
                  </div>
                ))}
              </div>
            </div>
